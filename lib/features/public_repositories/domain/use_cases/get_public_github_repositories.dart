...
@lazySingleton
class GetPublicGitHubRepositories implements UseCase<List<GitHubRepositoryEntity>, Params> {
  final PublicGitHubReposRepository repository;

  GetPublicGitHubRepositories(this.repository);

  @override
  Future<Either<Failure, List<GitHubRepositoryEntity>>> call(Params params) async {
    return await repository.getPublicGitHubRepositories(params.lastRepoId);
  }

  Stream<PublicGithubRepositoriesUiModel> getRepos(Params params) async* {
    yield* repository.getPublicGitHubRepositories1(params.lastRepoId);
  }
}
